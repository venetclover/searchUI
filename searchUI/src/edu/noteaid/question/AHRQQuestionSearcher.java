...
public class AHRQQuestionSearcher {
	String filename = "WebSource/ahrq/doc/";
	QuestionIndexer indexer;
	
	public AHRQQuestionSearcher() {
		indexer = new QuestionIndexer();
	}

	public AHRQQuestionSearcher(String path) {
		this.filename = path;
	}

	public void parseTitle(File f) {
		try {
			Document doc = Jsoup.parse(f, "UTF-8");
			Elements docs = doc.getElementsByClass("consumer");
			// StringBuilder sb = new StringBuilder();
			for (Iterator<Element> iterator = docs.iterator(); iterator
					.hasNext();) {
				Element p = iterator.next();
				String sentence = p.attr("title");
	...
	public ArrayList<QuestionAnsPair> search(File input) {
		...
		try {
			...
			Elements ps = doc.getElementsByTag("h4");
			// StringBuilder sb = new StringBuilder();
				...
				if (sentence.endsWith("?")) {
					...
					while (sibling != null) {
						if (sibling.tagName().equals("h3")
								|| sibling.tagName().equals("h4"))
							break;
						else {
							// System.out.println(sibling.text());
							ans.append(sibling.text());
							sibling = sibling.nextElementSibling();
						}

					}
					QuestionAnsPair qa_obj = new QuestionAnsPair(sentence,
							ans.toString());
					pairs.add(qa_obj);
				}
	...
	public static void main(String[] args) {
		AHRQQuestionSearcher qsearch = new AHRQQuestionSearcher();
		qsearch.startSearch();
	}
}
